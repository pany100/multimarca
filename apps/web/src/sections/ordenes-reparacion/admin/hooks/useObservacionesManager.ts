...
import { useUpdateObservacionesEntrada } from "./useUpdateObservacionesEntrada";

export const useObservacionesManager = () => {
  const { orden, setOrden } = useOrden();
  const { setSnackbar } = useSnackbarContext();
  const { updateObservacionesEntrada, loading } =
    useUpdateObservacionesEntrada();
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<string | null>(null);

  // Parse observaciones from JSON string
  const observacionesArray: string[] = orden?.observacionesEntrada
    ? JSON.parse(orden.observacionesEntrada)
    : [];

  // Helper to update observaciones via PATCH
  const updateObservaciones = async (newArray: string[]) => {
    try {
      const ordenActualizada = await updateObservacionesEntrada(
        orden.id,
