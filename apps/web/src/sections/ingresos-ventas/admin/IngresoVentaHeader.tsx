...
import { Box, Button, Chip, CircularProgress, Link as MuiLink, Typography } from "@mui/material";
...
function useHeaderSticky() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Hysteresis: activate at 120px, deactivate at 40px
...
function IngresoVentaHeader() {
  ...
  const ventaId = ingreso.venta?.id;
            ...
            {!isSticky && (
              <>
                <Typography variant="body2" color="text.secondary">
                  Fecha: {getFormattedDate(ingreso.fecha)} — Venta{" "}
                  <Link
                    href={`/dashboard/ventas/${ventaId}/ver`}
                ...
                <Box sx={{ display: "flex", gap: 3, mt: 1 }}>
                  ...
                  {ingreso.venta?.deuda !== undefined && (
                    <Typography
                      variant="body1"
                      fontWeight="medium"
                      color={ingreso.venta.deuda > 0 ? "error" : "success.main"}
                    >
                      Deuda venta:{" "}
                      <Typography
                        component="span"
                        fontWeight="bold"
                        color={
                          ingreso.venta.deuda > 0 ? "error" : "success.main"
                        }
                      >
                        {getFormattedPrice(ingreso.venta.deuda)}
                      </Typography>
                    </Typography>
                  )}
                </Box>
                <Link href="/dashboard/ingresos-ventas" passHref legacyBehavior>
                  <MuiLink
                    sx={{
                      display: "inline-flex",
                      alignItems: "center",
...
export default IngresoVentaHeader;
