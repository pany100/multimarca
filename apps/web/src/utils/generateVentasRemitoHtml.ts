...
export default function generateVentasRemitoHtml(
  venta: any,
  tipo: "original" | "duplicado" = "original",
  encabezadoPdf?: string,
): string {
  // Se mantiene el VO por compatibilidad (y para futuro), pero NO se usa para precios.
  ComprobanteCalculadoFactory.fromVenta(venta);

  const repuestos = (venta.repuestosUsados ?? []).filter(
    (el: any) => !el.ocultoParaCliente,
  );
  const terceros = venta.reparacionesDeTercero ?? [];

  const tipoLabel =
    tipo === "original"
      ? "ORIGINAL (Para el Cliente)"
      : "DUPLICADO (Para el Taller)";

  const itemRows = [
    ...repuestos.map(
      (el: {
        stock: { label: string; name: string; reportName?: string | null };
        unidadesConsumidas: number;
      }) =>
        `<tr>
          <td class="cell">${el.stock.label}</td>
          <td class="cell">${el.stock.reportName || el.stock.name}</td>
          <td class="cell cell-center">${el.unidadesConsumidas}</td>
        </tr>`,
    ),
    ...terceros.map(
        ...
        <div class="PDFPage">
          ...
          <div class="footer-section">
            <div class="observaciones-box">
              <div class="observaciones-title">Observaciones Remito Guia</div>
            </div>
            ...
            <div style="text-align: center; margin-top: 16px; font-size: 14px; font-weight: 700; letter-spacing: 2px;">
              ${tipoLabel}
            </div>
          </div>
        </div>
