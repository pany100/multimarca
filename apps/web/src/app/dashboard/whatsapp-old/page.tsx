...
function WhatsAppOldPage() {
  ...
  const fetchConversaciones = useCallback(async () => {
    try {
      const response = await authFetch("/api/notificaciones-whatsapp-old");
      if (response.ok) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await authFetch(
        `/api/notificaciones-whatsapp-old/${selectedConversacion.id}`,
        {
          method: "PUT",
  ...
  const handleDeleteConfirm = async () => {
    if (conversacionToDelete) {
      try {
        const response = await authFetch(
          `/api/notificaciones-whatsapp-old/${conversacionToDelete.id}`,
          {
            method: "DELETE",
          }
        );
  ...
  const handleConversacionClick = async (conversacion: Conversacion) => {
    ...
    try {
      const response = await authFetch(
        `/api/notificaciones-whatsapp-old/${conversacion.id}/marcar-leido`,
        {
          method: "PUT",
        }
      );
  ...
  return (
    <Box sx={{ p: 4 }}>
      <Typography variant="h4" gutterBottom>
        Conversaciones de WhatsApp OLD
      </Typography>
...
export default WhatsAppOldPage;
